...
//                            console.log(value);
    ...
    encryptSql(sql: string[]): string[] {
        return sql.map((item)=> {
            return CryptoJS.AES.encrypt(item, getBuhtaServerKey() + "AgFLsh23iGd").toString()
        });
    ...
    executeSQL(sql: SqlBatch): Promise<DataTable[]|string> {
        ...
        return new Promise<DataTable[]>(
            (resolve: (obj?: DataTable[]) => void, reject: (error: string) => void) => {
                ...
                cordovaHTTP.post(
                    "http://" + getBuhtaServerAddress(),
                    {data: JSON.stringify(req)},  // data
                    {}, // headres
                    (response: any) => {

                        let json: ExecuteSqlBatchSocketAnswer = JSON.parse(response.data);
